...
public class CreateSignatureContractRequestValidator : AbstractValidator<SignatureRequest>
{
    ...
    private static void ValidateOwnerClienteId(SignatureRequest request, ValidationContext<SignatureRequest> context)
    {
        ...
        foreach (var documento in request.Documentos)
        {
            ...
            foreach (var ownerId in documento.OwnerClientId)
            {
                ...
                {
                    context.AddFailure(new ValidationFailure(nameof(SignatureRequest.Documentos), MessageCatalog.GetErrorByCode(21007))
                    {
                        ErrorCode = "21007"
                    });
                    return;
